...
## CASE WHEN语句
case的第一个作用是判断的同时改变其值
```sql
SELECT username,
case when 'admin' then '管理员'
     when 'user' then '普通用户'
     else '其他用户' end role
from user;
```
...
可以使用触发器来进行审计跟踪，把修改记录到另外一张表中。MySQL 不允许在触发器中使用 CALL 语句 ，也就是不能调用存储过程。
...
> 好了这些基本上就是SQL语言的基本知识了，掌握之后就能应对简单的CRUD业务开发了。但是MYSQL还有很多知识点，比如索引的类别、锁、查询优化器、读写分离、主备一致、数据库引擎等等需要你去学习哦！
